...
var sketch = function(processing) {with(processing) {size(window.innerWidth, window.innerHeight);background(255);
...
function calculateMouseDistance() {
    ...
    mouseDistance = sqrt(Math.pow(mouseDistanceX, 2) + Math.pow(mouseDistanceY, 2));
    slope = mouseDistanceY / mouseDistanceX;
    tempAngle = Math.atan(slope) * 180 / Math.PI;
...
function edgeCheck() {
    ...
    if (circleY < 50) {
        edgeMovementY = 3;
    } else if (circleY > window.innerHeight - 50) {
        edgeMovementY = -3;
    } else {
        edgeMovementY = 0;
    }
...
function moveCircle() {
    ...
    if(mouseDistance < 100) {
        // fill(0, 0, 0);
        circleX += cos((angle + 180) * Math.PI / 180) * (300 / mouseDistance) + edgeMovementX;
        circleY += sin((angle + 180) * Math.PI / 180) * (300 / mouseDistance) + edgeMovementY;
    } else {
